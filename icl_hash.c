 ...
 * @file icl_hash.c
 *
 * @brief Dependency free hash table implementation, slightly modified by Remo Andreoli to implement section lock to guarantee no race conditions
 *
...
//locka la sezione in cui appartiene hash_val 
void lockSection(icl_hash_t* ht, unsigned int hash_val){
	int sectionDim=(HASHTABLE_SIZE/HASHTABLE_REGION); //quante entry in una regione
	int i=floor(hash_val/sectionDim); //index mutex associato alla regione
	
	Pthread_mutex_lock(&(ht->mtx[i]));
}

//unlocka la sezione in cui appartiene hash_val
void unlockSection(icl_hash_t* ht, unsigned int hash_val){
	int sectionDim=(HASHTABLE_SIZE/HASHTABLE_REGION); //quante entry in una regione
        int i=floor(hash_val/sectionDim); //index mutex associato alla regione

        Pthread_mutex_unlock(&(ht->mtx[i]));
}
...
icl_hash_create( int nbuckets, unsigned int (*hash_function)(void*), int (*hash_key_compare)(void*, void*) )
{
    ...
    //inizializzazione mutex delle regioni
    for(i=0;i<HASHTABLE_REGION;i++){
    	Pthread_mutex_init(&(ht->mtx[i]), NULL);
    }
...
icl_hash_find(icl_hash_t *ht, void* key)
{
    ...
    lockSection(ht, hash_val);

    for (curr=ht->buckets[hash_val]; curr != NULL; curr=curr->next){
        if ( ht->hash_key_compare(curr->key, key)){
            unlockSection(ht, hash_val);
	    return(curr->data);
	}
    }
...
icl_hash_destroy(icl_hash_t *ht, void (*free_key)(void*), void (*free_data)(void*))
{
    ...
    //Distruzione mutex delle regioni
    for(i=0;i<HASHTABLE_REGION;i++){
		Pthread_mutex_destroy(&(ht->mtx[i]));
    }
...
/**
 * @function freeNick
 * @brief free del nick, usata in fase di chiusura dell'hashtable
 *
 * @param nick	 puntatore al buffer 
 *
 */
void freeNick(void* nick){
        free((char*) nick);
}

/**
 * @function freeHistory
 * @brief free della history, usata in fase di chiusura dell'hashtable
 *
 * @param data	 puntatore alla struct User
 *
 */
void freeHistory(void* data){
	destroyHistory((qHistory*) data);
}
