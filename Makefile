...
# 
FILE_DA_CONSEGNARE=chatar.sh chatty.c client.c config.h connections.h \
                   connections.c DATA/chatty.conf1 DATA/chatty.conf2 \
                   historyQueue.h historyQueue.c icl_hash.h icl_hash.c \
                   listener.c Makefile message.h ops.h parser.c \
                   pthreadUtils.h pthreadUtils.c queue.h queue.c \
                   server.h stats.h threadPool.c worker.c utils.h \
		   testfile.sh testconf.sh teststress.sh testleaks.sh \
		   Documentazione/Relazione.pdf Documentazione/html \
# inserire il nome del tarball: es. NinoBixio
TARNAME=RemoAndreoli
